...
void AsyncWebSocket::_handleEvent(AsyncWebSocketClient *client,
                                  AwsEventType type,
                                  void *arg,
                                  uint8_t *data,
                                  size_t len) {
...
void AsyncWebSocket::_cleanBuffers() {
  ...
  while (_buffers.remove_first([](AsyncWebSocketMessageBuffer *const &buffer) {
    return (buffer != nullptr) && buffer->canDelete();
  }));
}
